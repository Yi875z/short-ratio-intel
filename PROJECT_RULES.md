...
- **データソースと取得条件**:
  - 空売り比率: JPX公開PDF（`jpx_pdf_client.py`）が第一候補、stock-marketdata.com のスクレイパー
    ...
    **JPXは月ごとのページに、その月の全営業日ぶんのPDFを載せる**（運用として恒常的にそうなっている。
    いいものさんの実務知識。時点の観測ではなく公開運用そのものなので、これを前提にしてよい）。
    当月ぶんは一覧ページ、過去は月別アーカイブに12ヶ月ぶん
    （`00-archives-01.html` … `-12.html`。01が前月、12が13ヶ月前。13以降は404）。
    2026-09-03 の実測もこれと一致した（一覧=9/01〜9/03、archives-01=8月20日・-02=7月22日・
    -03=6月22日。いずれも当DBが持つその月の営業日と差分ゼロ）。
    したがって**直近13ヶ月ぶんの内訳はいつでも取り直せる**。
    月末に一覧が当月ぶんを保持しているかを別途検証する必要はない（上記の運用が前提のため）。
    ...
    **欠測を見つけたら「もう取れない」と判断する前に必ずアーカイブを見に行くこと**
    （2026-09-03、月初に一覧ページだけを見て「直近2営業日しか無い＝復旧不能」と誤断した。
    9月の営業日がまだ2日しか無かっただけで、一覧は当月ぶん全部を載せていた）。
    内訳の欠測は `pipeline_health.check_breakdown_gaps` で検出する。
    スクレイパー側は内訳を持たず0を返す。**0で既存の内訳を上書きしないこと**
    （2026-04-20〜08-31 の22営業日ぶんの内訳が実際に潰れていた。実測ではこの22日は
    5つの塊で、すべてその月の最終営業日で終わっている＝月替わりで前月末ぶんが
    一覧から外れ、スクレイパーへ落ちた形。実行ログが無いため断定はしない）。
...
- **自己点検と気づく経路**: `pipeline_health.collect_health_issues()` を
  ...
  **今日動けば直せて、今日動かないと取り返しがつかないものだけを critical にする**
  内訳欠測でこれに当たるのは**当日ぶんだけ**で、理由は「データが失われるから」ではなく
  「JPX公式PDFに到達できていない＝取得経路が壊れているサイン」だから。
  過去日の欠測はバックフィルで埋められるので `high` に留め、パイプラインは落とさない。
