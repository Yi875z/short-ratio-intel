...
- **指数の値動き**: 日経平均の四本値は Yahoo チャートAPI `^N225`、日経平均・TOPIXの終値は nikkei225jp.com。
  **TOPIXの四本値は無料・認証不要の範囲に存在しない**（詳細は下記「指数チャートのデータ源」）。
- **AI**: Gemini API。既定モデルは `gemini-3.7-flash`（2026-08-25〜・試験運用中。失敗時は 3.6 → 3.5 へ自動退避）
...
### 指数チャートのデータ源（日経平均・TOPIX）

概要タブと業種タブの空売りチャート直下に、同じ期間の日経平均・TOPIXを併置している
（`_render_index_charts`）。日経平均はライン／ロウソク足を切り替えられるが、**TOPIXはライン専用**。

理由は TOPIX の四本値がどこにも無いため。2026-08-25 に調査した結果は次のとおり。

| ソース | 日経平均 | TOPIX |
|---|---|---|
| Yahoo チャートAPI | `^N225` で四本値が取れる | `^TPX` / `^TOPX` / `998405.T` すべて空 |
| nikkei225jp.com 日次JSON | 終値のみ（長期） | **終値のみ（実TOPIX・長期）** |
| stock-marketdata.com/nikkei225.html | 終値のみ・直近33営業日 | 終値のみ・直近33営業日 |
| stooq | JS検証ページ化して死亡 | 同左 |

TOPIX連動ETF（`1348.T` / `1306.T`）なら四本値は取れるが、縦軸がETF価格になり
TOPIXポイントと別物になるため採用していない。実TOPIXの時系列が取れるのは
nikkei225jp.com だけなので、この依存は簡単には外せない。

⚠️ **nikkei225jp.com の足は 15:00 UTC（＝翌 00:00 JST）でスタンプされている**。
生の ms をそのまま日付にすると取引日が1日前にずれる。変換には必ず `_jst_date()` を使うこと
（2026-08-25 に `_fetch_site_nikkei_topix()` で実際にずれていたのを修正。
8/21終値 66,016.36 が 8/20 として入っていた）。
...
| モデル | 結果 | 所要 | 備考 |
|---|---|---|---|
| `gemini-3.6-flash` | OK | 61.7秒 | 生JSON 13,319字・スキーマ検証通過。タイムアウト180秒の半分（90秒）に収まる |
| `gemini-3.7-flash` | NG | — | この試行は 503 high demand。ただし 8/23 の検証は 85.5秒で成功、8/25 19:39 の定時実行も成功 |

3.7 は「常に駄目」ではなく**当たり外れが大きい**。上表は1回ぶんの記録であって判定ではない。

**2026-08-25 の判断: 3.7-flash を既定に戻して試験運用する。**
3.7 は 504連発 / 600秒超 / 503高需要と3度別々の壊れ方をしているが、いずれも
「遅い・混む」であって出力が壊れるわけではない。そして 8/24 の致命傷だった
「SDK の内部リトライが日次枠を食い潰す」経路は塞いだので、3.7 が失敗しても
3.6 へ自動退避してレポート自体は出る。品質は未測定なので、新しいモデルの質を
実運用で評価する価値が上回ると判断した。

**恒久採用の判断材料**は2つ。`ai_reports.model_used` に退避が頻発していないか、
`check_gemini_model` を数日回して所要時間のばらつきがタイムアウトの半分（90秒）に
収まるか。退避が目立つなら `GEMINI_MODEL_DEFAULT` を 3.6 に戻す（1行）。

⚠️ `GEMINI_REQUEST_TIMEOUT_SEC` と `daily_fetch.yml` の `timeout-minutes` は連動する。
片方だけ伸ばすと退避先へ到達する前に job が切れてレポートが欠落する。
現在は 180秒 × 3回 × 3モデル ≒ 27分 に対し job 側 35分。テストで突き合わせている。
