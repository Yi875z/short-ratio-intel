...
- **AI**: Gemini API。既定モデルは `gemini-3.7-flash`（`GEMINI_MODEL` で変更可。2026-07-25 に 3.5→3.6、2026-08-23 に 3.6→3.7 へ移行）
...
（既定 `gemini-3.7-flash` ⇄ 退避先 `gemini-3.6-flash`。両方とも本番キーで動作確認済み）。
