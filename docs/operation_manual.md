# 空売り比率インテリジェンス 運用マニュアル

最終更新: 2026-05-28

---

## システム概要

JPXの空売り集計データと業種別空売り比率を取得・分析し、現在の市場テーマを踏まえたGemini AIレポートを生成するStreamlitアプリ。

- **URL**: http://localhost:8501
- **データソース**: JPX公式空売り集計PDF、stock-marketdata.com（フォールバック）
- **AI**: Gemini API（AIレポートタブ）。既定モデルは `gemini-3.5-flash`（`.env` の `GEMINI_MODEL` で変更可）
- **任意ニュース取得**: Tavily API（市場テーマタブ / AIレポート生成時に明示ON）

---

## 起動方法

### ワンクリック起動（推奨）

`C:\CarSol\short-ratio-intel\アプリ起動.bat` をダブルクリックする。

- ポート競合が発生していても自動解消して起動する
- 起動後、ブラウザが自動的に http://localhost:8501 を開く
- **黒いウィンドウ（コマンドプロンプト）を閉じるとアプリも停止する**

### コマンドで起動する場合

```powershell
cd C:\CarSol\short-ratio-intel
python -m streamlit run app\streamlit_app.py
```

---

## 停止方法

`アプリ起動.bat` を実行して開いた黒いウィンドウを閉じる。またはそのウィンドウで `Ctrl+C` を押す。

---

## 日常運用の流れ

1. `アプリ起動.bat` をダブルクリックしてアプリを起動する
2. ブラウザが開いたら、必要に応じて「取得見込みチェック」で指定日の公開状況を確認する
3. 「データ取得」ボタンでデータを更新する
4. 各タブ（概要 / 業種 / JPX内訳 / 市場テーマ / AIレポート / 履歴）で分析を確認する
5. 市場テーマタブで手動メモや任意のニュース取得を使い、主要テーマ候補を保存する
6. AIレポートタブでGeminiによる相場考察を生成し、品質チェック欄を確認する
7. 確認が終わったら黒いウィンドウを閉じてアプリを停止する

---

## データ取得見込みチェック

左メニューの取得日を指定し、「取得見込みチェック」から「指定日の取得可否を確認」を押すと、DBへ保存する前に公開元の状態を確認できる。

主な確認内容:

- 指定日がDBに保存済みか
- JPX公式PDFで業種別データと東証全体データを確認できるか
- JPX公式PDFが未公開の場合、stock-marketdata.comのフォールバックで取得できるか
- 業種別データと東証全体データが両方そろっているか

判定が「取得可能」の場合は、「指定日を取得」でDBへ保存できる見込みが高い。
「一部取得可能」または「未公開または取得不可」の場合は、公開待ち、非営業日、通信制限、公開元サイト変更の可能性があるため、時間を置いて再確認する。

---

## AIレポート品質チェック

AIレポートタブでは、保存済みレポートの上部に「AIレポート品質チェック」が表示される。

主な確認内容:

- 必須セクション（市場テーマ、JPX内訳、シグナル履歴、投資判断ガードレール等）が含まれているか
- 「売買推奨ではない」「日次フロー」などの安全表現が入っているか
- 「確信的」「必ず上がる」「反発確率」などの過剰断定表現がないか
- VIX、WTI、SOX、GEX、米金利、ドル円などの未確認データを事実として断定していないか
- 構造化JSONに市場テーマ、確認条件、誤判定リスク、追加確認データが保存されているか
- 市場テーマ履歴・転換メモの主要テーマ名や変化状態がレポートに反映されているか

判定が「要修正」または「要確認」の場合は、表示された行の `message` と `evidence` を確認し、必要に応じて市場メモを補足してAIレポートを再生成する。
同じ日付のAIレポートを再生成する場合、前回保存レポートの品質チェック失敗項目が「前回品質チェックからの改善指示」として自動的にGeminiプロンプトへ渡される。
AIレポートタブでは、再生成前に「再生成時にGeminiへ渡す改善メモ」をプレビューでき、チェックを外すとその再生成では改善メモを使わない。
品質チェック結果は、全チェック項目のCSVと、改善メモを含むMarkdownとしてダウンロードできる。
同じ日付のAIレポートを再生成した直後は、「前回再生成の品質比較」として再生成前後のスコア、未通過項目数、重大項目数の差分を確認できる。
品質比較レビューは `data/reports/ai_report_quality_comparison_YYYY-MM-DD.md` に保存され、セッションを開き直してもAIレポートタブから再確認できる。

履歴タブでは「AIレポート品質履歴」として、保存済みレポートの日付別スコア、判定、重大項目数、未通過項目数を一覧で確認できる。
品質履歴はCSVとしてダウンロードできるため、再生成すべき日付の優先順位付けに使える。

---

## 市場テーマ履歴

市場テーマタブでは、保存済みテーマ判定の履歴を確認できる。

主な確認内容:

- 選択日と前回保存日のテーマスコア比較
- 新規、強化、継続、弱体化、消滅の状態分類
- 直近30件の保存日を対象にした市場テーマスコア推移
- テーマ別の関連業種、根拠数、未確認データ数

テーマが変わったときは、先に市場テーマタブで手動メモまたはニュース取得を使って判定を保存し、その後AIレポートを生成する。

AIレポート生成時には、この市場テーマ履歴から「市場テーマ履歴・転換メモ」が自動でプロンプトに挿入される。対象日の保存済みテーマがない場合は、AIレポート生成時の手動メモやニュース文脈から一時的にテーマ判定を作り、前回保存テーマと比較する。この一時判定はDBには保存されない。

---

## ファイル構成（重要箇所のみ）

```
C:\CarSol\short-ratio-intel\
├── アプリ起動.bat                      ← ワンクリック起動ファイル
├── app\
│   ├── streamlit_app.py               ← アプリ本体（通常のPythonソース）
│   └── __pycache__\
│       └── streamlit_app_original.cpython-311.pyc  ← 旧復旧用バックアップ
├── src\                               ← データ取得・分析・AIエンジン
├── config\                            ← 設定ファイル
├── data\                              ← SQLiteデータベース（Gitで除外）
└── .env                               ← APIキー（Gitで除外）
```

---

## 重要な注意事項

### `streamlit_app_original.cpython-311.pyc` について

現在の `app\streamlit_app.py` は通常のPythonソースとして保守できる状態に復旧済み。
`app\__pycache__\streamlit_app_original.cpython-311.pyc` は旧復旧用バックアップとして残しているが、通常起動では使用しない。

背景: 2026-05-07、PowerShellの文字コード変換バグにより `streamlit_app.py` のソースコードが破損した。2026-05-19に通常ソースへ復元済み。

### `.env` ファイルについて

APIキーが記載された `.env` は Gitで管理されていない。PCを移行する際は手動でコピーすること。

---

## トラブルシューティング

### ポート8501がすでに使用中と表示される

`アプリ起動.bat` が自動的に解消する。手動で解消する場合は以下を実行する。

```powershell
# ポート8501を使っているPIDを確認
netstat -aon | findstr ":8501"
# 該当PIDを終了（例：PID=12345）
taskkill /PID 12345 /F
```

### アプリは起動するが「データなし」と表示される

データ取得ボタンを押してJPX公式PDFまたはstock-marketdata.comからデータを取得するか、DBにデータが存在するか確認する。

### AIレポートが生成されない

`.env` に `GEMINI_API_KEY` が設定されているか確認する。Tavilyニュース取得を使う場合だけ `TAVILY_API_KEY` も必要。
生成後に品質チェックが「要確認」になる場合は、市場メモやニュース材料を補足して再生成する。

使用モデルは既定で `gemini-3.5-flash`。`.env` に `GEMINI_MODEL=別モデル名` を書けば切替できる（旧モデルへ戻す場合は `GEMINI_MODEL=gemini-3-flash-preview`）。`gemini-3.5-flash` は無料枠が1日20リクエスト（20 RPD）のため、レポート再生成・品質比較を1日に何度も回すとクォータ枯渇しうる。枯渇時は同じモデルを叩き直さず24時間待つ。

---

## バックアップ運用

このプロジェクトはGit管理下にある（2026-05-08 初回コミット済み）。コードを変更した後は必ず以下を実行する。

```powershell
cd C:\CarSol\short-ratio-intel
git add -A
git commit -m "変更内容の説明"
```

GitHubへのリモートバックアップは任意のタイミングで設定する。設定済みであれば `git push` も実行する。
