...
# 一覧ページは「当月ぶんの全営業日」を載せる。過去の月は月別アーカイブへ移る。
# 12ヶ月ぶんが保持され、01 が前月、12 が13ヶ月前に対応する。13以降は404。
#
# 2026-09-03 実測: 一覧=2026/09/01〜09/03（当日はPDF未公開で行だけ）、
# archives-01=2026/08/03〜08/31 の20日、-02=7月22日、-03=6月22日。
# いずれも当システムDBが持つその月の営業日と完全一致した（差分ゼロ）＝
# 月の一部ではなく全営業日が載っている。
# したがって直近13ヶ月ぶんはいつでも取り直せる。欠測を諦める前に必ずここを見ること。
_ARCHIVE_URL = (
...
class JPXShortSellingClient:
    ...
    def _get_pdf_url_map(self) -> dict[tuple[str, str], str]:
        ...
        try:
            ...
            # スクレイパーは内訳を持たない（0を返す）ため、内訳が失われる方向に働く。
