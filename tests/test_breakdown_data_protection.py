...
2026-04-20〜08-31 の22営業日ぶんの内訳が 0 に潰されていた事故の再発防止。

実測（2026-09-03）:
  - 一覧ページは当月ぶんの全営業日のみを載せる。前月以前は月別アーカイブへ移る。
  - 潰れていた22日は5つの塊で、すべてその月の最終営業日で終わっていた
    （4/30・5/29・6/30・7/31・8/31）。

壊れ方（コード上で確認した経路）:
  1. 一覧に無い日付は PDF を引けず stock-marketdata へフォールバックする。
     スクレイパーは内訳を持たず 0 を返す。
     一覧ページの取得失敗を空辞書としてキャッシュする不具合も、同じ状態を作る。
  2. upsert が無条件に上書きしたため、取得済みの正しい内訳が 0 に潰された。
  3. 画面は内訳から比率を再計算していたため「空売り比率 0%」と表示された。
"""
